...
any changes made without a keyup don't update the string
                    ...
                    <React.Fragment key={`tni-${i}`}>
                        {toolName.map((toolNameInner, j) => (
                            <React.Fragment key={`tni-${j}`}>
                                <button onClick={() => setSelectionDropTool({ tool: toolNameInner })} title={capitalize(toolNameInner.split('-').join(' '))} className={`${toolNameInner}${checkables.includes(toolNameInner) && toolsState[camelCase(toolNameInner)] ? ' checked' : ''}`}>{tools.allIcons[i][j]}</button>
                            </React.Fragment>
                        ))}
